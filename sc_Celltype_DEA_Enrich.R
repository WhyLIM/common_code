...
# 基础目录设置 - 只需修改这部分
base_dir <- "~/scLC"
project_name <- "W_Heart"  # 项目名称
clustering_resolution <- "0.8"  # 聚类分辨率
group_var <- "ftype"  # 分组变量（细胞类型）
condition_var <- "mfield"  # 条件变量（实验条件）
ident_1 <- "L"  # 实验组
ident_2 <- "D"  # 对照组
...
# 创建所需目录
for (dir in c(dea_dir, enrich_dir, volcano_dir)) {
  if (!dir.exists(dir)) {
...
extract_top_degs <- function(degs, n = 10) {
  result <- lapply(names(degs), function(celltype) {
    ...
    # 添加细胞类型和调控方向信息
    bind_rows(
      mutate(up, celltype = celltype, regulation = "Up"),
      mutate(down, celltype = celltype, regulation = "Down")
    )
  }) %>% bind_rows()
...
# 保存网络图
save_network_plot <- function(cnet_data, prefix, title_prefix, dimensions = c(9, 8)) {
  for (n in names(cnet_data)) {
    safe_name <- gsub("[/ ]", "_", n)
    filename_png <- paste0(prefix, "_", safe_name, ".png")
    filename_pdf <- paste0(prefix, "_", safe_name, ".pdf")
...
# ======================================= 数据加载 =======================================
setwd(analysis_dir)
seurat_object <- readRDS("seurat_celltype.rds")
...
for (celltype_name in names(seurat_cells)) {
  ...
  # 设置分组标识为条件变量
  Idents(subset_obj) <- subset_obj[[condition_var]]
...
# 保存合并的差异基因结果
write.csv(combined_degs, "DEGs_Celltype_W_Heart.csv", row.names = FALSE)

# 4. 筛选显著差异基因
sig_markers <- combined_degs %>% filter(p_val_adj < deg_pval_cutoff)

# 5. 分别提取上下调基因并按细胞类型保存为Excel
...
create_excel_with_sheets(up_genes, "marker_genes_up.xlsx")
create_excel_with_sheets(down_genes, "marker_genes_down.xlsx")

# 6. 提取每个细胞类型中前N个上下调基因并保存
top_degs_df <- extract_top_degs(
  sig_markers %>% split(.$celltype),
  n = 10
)
write.csv(top_degs_df, "DEGs_top10.csv", row.names = FALSE)
...
for (ct in names(degs_for_enrichment)) {
  ...
  kegg_results[[paste0(ct, "_up")]] <- KEGGEnrichment(
    DEG_up, 
    csvBaseName = paste0("_", safe_name)
  )
...
config_info <- list(
  ...
  comparison = list(ident_1 = ident_1, ident_2 = ident_2),
  parameters = list(
...
saveRDS(config_info, "dea_analysis_config.rds")
message("分析完成! 结果保存在: ", enrich_dir)